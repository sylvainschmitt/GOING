...
A Bayesian method was used to infer parameters using `stan` language [@Carpenter2017] and the `rstan` package [@StanTeam2018] in the R environment [@RCoreTeam2020] using the No-U-Turn Sampler algorithm [NUTS, @Hoffman2014], which performs better for estimating genetic parameters and breeding values [@Nishio2019].
...
>  Non-adaptive population genetic variation is expected to be spatially and temporally structured; observing these patterns cannot be used to infer an adaptive component to genomic variation, without adequately accounting for historical relatedness/structure. Unfortunately, it is very challenging to do so convincingly, even in deeply sampled datasets from model systems. The analyses used in this study (including KING) are insufficient to convincingly show that population structure has been adequately accounted for when inferring adaptive differences in this specific dataset.

If the observed genetic variation related with $NCI$ is non-adaptive, *i.e.* confounded with neutral processes, the observed patterns should not differ from randomized $NCI$ maps across study plots keeping individuals spatial structure while breaking their relation to $NCI$. On the opposite, if the observed pattern significantly different to patterns observed randomizing $NCI$ maps across plots, the relation is adaptive and non neutral.

We need:

* [x] krigging of NCI maps per plot
* [ ] pivot the NCI plot maps in the four directions
* [ ] extract the N random maps
* [ ] run models
* [ ] compared true genetic variance to random ones

```{r ncikriged, eval=F}
library(gstat)
library(sf)
krige_stand <- function(name){
  stand <- vroom::vroom("save/nciall.tsv") %>% 
    mutate(stand = paste0("P", Plot, "C", SubPlot)) %>% 
    filter(CensusYear == 2015) %>% 
    filter(stand == name)
  if(name == "P15C1")
    stand <- filter(stand, Xfield < 125)
  if(max(stand$Xfield > 125)){
    stand$Xfield <- stand$Xfield - 125
  } else {
    stand$Xfield <- stand$Xfield - 20
  }
    
  if(max(stand$Yfield > 125)) {
    stand$Yfield <- stand$Yfield - 125
  } else {
    stand$Yfield <- stand$Yfield - 20
  }
  stand.xy  <- as_Spatial(st_as_sf(stand, coords = c("Xfield", "Yfield")))
  stand.xy <- stand.xy[-sp::zerodist(stand.xy)[,1],]
  grd <- expand.grid(Xfield = seq(0, 105, length.out = 105), Yfield = seq(0, 105, length.out = 105)) %>% 
    st_as_sf(coords = c("Xfield", "Yfield")) %>% 
    as_Spatial()
  stand.vgm <- variogram(NCI ~ 1, stand.xy)
  stand.fit <- fit.variogram(stand.vgm, model = vgm("Sph"))
  # plot(stand.vgm, stand.fit)
  stand.kriged <- krige((NCI) ~ 1, stand.xy, grd, model = stand.fit)
  stand.kriged %>% 
    as.data.frame %>% 
    rename(Xfield = coords.x1, Yfield = coords.x2, NCI = var1.pred)
}
stands <- vroom::vroom("save/nciall.tsv") %>% 
  mutate(stand = paste0("P", Plot, "C", SubPlot)) %>% 
  dplyr::select(stand) %>% 
  collect() %>% 
  unique() %>% 
  unlist()
t <- lapply(stands, krige_stand)
names(t) <- stands
bind_rows(t, .id = "stand") %>% 
...
```{r plotnci, eval=F}
library(gganimate)
g <- vroom::vroom("data/ncikriged.tsv") %>% 
  # filter(stand == "P15C3") %>%
  ggplot(aes(Xfield, Yfield, fill = NCI)) + 
  geom_raster() +
  coord_equal() +
...
anim_save("save/nci.gif", g)
```

```{r plotncigif, fig.cap="NCI in 2015 across SubPlots."}
include_graphics("save/nci.gif")
```


```{r rotations, eval=F}
rasters <- vroom::vroom("data/ncikriged.tsv") %>%
  group_by(stand) %>%
  do(raster = rasterFromXYZ(dplyr::select(., Xfield, Yfield, NCI)))
lapply(rasters$raster, plot)
plot(rasters$raster[[1]])
plot(flip(rasters$raster[[1]], direction = 1))
plot(flip(rasters$raster[[1]], direction = 2))
plot(t(rasters$raster[[1]]))
```

## Closely-related species

> Lastly, it's unclear whether this study is truly 'within-species', as there appear to be three closely related species simultaneously included in the analysis.
...
\begin{equation} 
  y_{p,i} \sim \mathcal{logN}(log(a_{p,i}),\sigma_1) \\
  a_{p,i} \sim \mathcal{MVlogN_N}(log(\mu),\sigma_2.K)
  (\#eq:animalenv)
\end{equation} 

where individual is defined as a normal law centered on the individual genetic additive effects $a$ and associated individual remaining variance $\sigma_R$. Additive genetic variance $a$ follows a multivariate lognormal law centered on the population mean $\mu$ of covariance $\sigma_G K$.

We fitted the equivalent model with following priors:

\begin{equation} 
  y_{p,i} \sim \mathcal{logN}(log(\mu) + \hat{\sigma_2}.A.\epsilon_i, \sigma_1) \\ 
  \epsilon_i \sim \mathcal{N}(0,1) \\ 
  ~ \\
  \mu_p \sim \mathcal{logN}(log(1),1) \\  
  \sigma_1 \sim \mathcal N_T(0,1) \\ 
...
```{r envGenoSpeciesVarPart, fig.cap="Genetic variance partitioning for environmental variables."}
