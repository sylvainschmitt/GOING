...
```{r vgdist, fig.cap="Among-genotype variance (VG)  between observed value (red) and values observed in null models (black) randomizing neighbourhood crowding index (NCI) among plots."}
# vgs <- lapply(fit, broom.mixed::tidyMCMC, c("Vg"), droppars = NULL, rhat = T) %>%
  # bind_rows(.id = "simulation")
vgs <- data.frame(estimate = rnorm(101), simulation = 0:100)
vgs$estimate[1] <- 3
filter(vgs, simulation != 0) %>% 
  ...
  xlab(expression(V[G])) +
  ggtitle("Fake graph waiting for results", "P < 0.01")
```
