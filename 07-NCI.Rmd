...
* [x] pivot the NCI plot maps in the four directions
* [x] extract the N random maps
* [ ] run models
...
```{r envGenoNullTab, fig.cap="Genetic variance of individual growth potential with a lognormal animal model.", eval=F}
fitEnvNull <- list(NCI = list())
for(var in c("NCI")){
  for(sim in list.files("save/EnvGeno", 
                        pattern = var, full.names = T)){
    load(sim)
...
```{r eval=F}
vgs <- lapply(fit, broom.mixed::tidyMCMC, c("Vg"), droppars = NULL, rhat = T) %>% 
  bind_rows(.id = "simulation")
vgs$estimate <- rnorm(3)
filter(vgs, simulation != 0) %>% 
  ggplot(aes(estimate)) +
  geom_density() +
  geom_vline(xintercept = filter(vgs, simulation == 0)$estimate, col = "red") +
  xlab(expression(V[G]))
```
