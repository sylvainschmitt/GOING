...
```{r PicoGreen2, fig.cap="Reamplified samples dosed using PicoGreen (concentration in ng.uL)."}
picogreen2 <- read_csv(file.path(path, "QuantIt", 
...
```{r P6reorganizationFig, fig.cap="Original position of plates 6.1 and 6.2 reorganized plate 6.3."}
repetitions %>% 
...
re-extracted samples from P6 (P6.2) have been pooled into a single separated pool for the capture reaction (in total, we do 16 capture reactions on pooled samples, therefore we still have to constitute the 15 remaining pools).
All other samples have been pooled by batches of 32 using the plate order illustrated in FIgure \@ref(fig:captureReactions).
We wanted 100 to 500 ng of DNA per reaction, and the reaction with the lowest number of samples had 16 samples. 
Consequently we used 15 ng of each sample, resulting in 240 to 645 ng of DNA per pool (but we may lose material in purification, so we aimed for extra). 
...
```{r NanoDropC1Fig, fig.cap="DNA content of library pools assessed by NanoDrop."}
nanodrop %>% 
  mutate(row = substr(Position, 1, 1)) %>% 
  mutate(col = substr(Position, 2, 3)) %>% 
  ggplot(aes(y = factor(row,levels = rev(LETTERS[1:8])),
...
We had a heterogeneity of fragment size distribution among pools with a large spectrum (Fig. \@ref(fig:TapeStation)). 
We used a TapeStation to select fragments between 330 and 700 bp.
...
data.frame(
  ...
  filter(!is.na(DNA)) %>% 
  kable(caption = "Concentrations for pools of library pools.",
...
We then pooled back the capture reactions, assessed their concentration by qPCR and adjusted final samples for sequencing by an equimolar pooling Pools 1 and 2, and 3 and 4 together. 
